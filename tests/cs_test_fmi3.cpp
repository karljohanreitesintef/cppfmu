...
extern "C" void logger(fmi3InstanceEnvironment, fmi3Status status,
                       fmi3String category, fmi3String message) noexcept {
...
int main() {
  ...
  const auto instance = fmi3InstantiateCoSimulation(
      TEST_INSTANCE_NAME, "04b947f3-c057-4860-b59b-eb0bd6fa52be", "", fmi3False,
      fmi3True, fmi3False, fmi3False, nullptr, 0, nullptr, &logger, nullptr);
  assert(instance);

  // Initialization (FMI 3.0)
  {
    const auto rc = fmi3EnterInitializationMode(instance, fmi3False, 0.0, 0.0,
                                                fmi3False, 0.0);
    assert(rc == fmi3OK);
  }
  {
  ...
  // Derivative APIs — default "not supported" path (FMI 3.0)
  {
    ...
    auto rc = fmi3GetDirectionalDerivative(instance, vr, 1, vr, 1, dvKnown, 1,
                                           dvUnknown, 1);
    assert(rc == fmi3Error);
  }
  {
  ...
  // fmi3GetOutputDerivatives — success, value = seed * order = 2.0 * 1 = 2.0
  // (FMI 3.0)
  {
    const fmi3ValueReference vr[] = {0};
    fmi3Int32 order[] = {1};
  ...
  // fmi3GetDirectionalDerivative — success, dvUnknown = seed * dvKnown = 2.0
  // * 5.0 = 10.0 (FMI 3.0)
  {
    ...
    fmi3Float64 dvUnknown[] = {0.0};
    auto rc = fmi3GetDirectionalDerivative(instance, vUnknown, 1, vKnown, 1,
                                           dvKnown, 1, dvUnknown, 1);
    assert(rc == fmi3OK);
  ...
  // Simulation (FMI 3.0)
  {
    ...
    const auto rc =
        fmi3DoStep(instance, 0.0, 0.1, fmi3False, &eventHandlingNeeded,
  ...
  {
    const auto rc = fmi3DoStep(instance, 0.0, 0.1, fmi3False, nullptr, nullptr,
                               nullptr, nullptr);
    assert(rc == fmi3OK);
  }
  {
  ...
  {
    const auto rc =
        fmi3DeserializeFMUState(instance, serializedState.data(),
                                serializedState.size(), &restoredState);
    assert(rc == fmi3OK);
  ...
  // Test fmi3GetAdjointDerivative (error path, not supported)
  {
    ...
    const auto rc = fmi3GetAdjointDerivative(instance, vr, 1, vr, 1, seed, 1,
                                             sensitivity, 1);
    assert(rc == fmi3Error);
  }
  ...
  // Test fmi3GetVariableDependencies (error path, not supported)
  {
    ...
    const auto rc = fmi3GetVariableDependencies(
        instance, 0, elementIndicesOfDependent, independents,
        elementIndicesOfIndependents, dependencyKinds, 10);
    assert(rc == fmi3Error);
  }
  ...
  }
  {
    ...
    const auto rc = fmi3UpdateDiscreteStates(instance, &needUpdate, &termSim,
                                             &nominalsChanged, &valuesChanged,
                                             &timeDefined, &time);
    assert(rc == fmi3OK);
  ...
  // then allows Reset from Terminated, which returns the instance to
  // Instantiated.
